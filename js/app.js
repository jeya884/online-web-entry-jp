...
function normalizeRow(row){const map={};Object.keys(row).forEach(k=>map[k.toLowerCase().replace(/[^a-z0-9]/g,"")]=row[k]);const get=(...keys)=>{for(const k of keys){const v=map[k.toLowerCase().replace(/[^a-z0-9]/g,"")];if(v!==undefined&&v!==null)return v}return ""};return {name:String(get("Name","Stock Name","Symbol")).trim(),date:get("Date & Time","Date","DateTime"),action:String(get("Action")).trim().toUpperCase(),exchange:String(get("Exchange")).trim(),orderQty:num(get("Order Qty","Quantity","Qty")),executionQty:num(get("Execution Qty","Executed Qty","ExecutionQty","Qty")),disclosedQty:num(get("Discl. Qty","Disclosed Qty","Discl Qty")),orderPrice:num(get("Order Price","Price","Buy Price","Sell Price")),triggerPrice:num(get("Trigger Price")),orderType:String(get("Order Type")).trim(),product:String(get("Product")).trim(),status:String(get("Status")).trim(),reason:String(get("Reason")).trim()}}
...
$("importExcelBtn").onclick=async()=>{const file=$("excelFile").files[0];if(!file)return alert("Choose an Excel or spreadsheet file first");if(!window.XLSX)return alert("Spreadsheet import library did not load. Check internet connection and reload.");try{const buf=await file.arrayBuffer();const wb=XLSX.read(buf,{type:"array",cellDates:true});let rows=[];wb.SheetNames.forEach(s=>rows.push(...XLSX.utils.sheet_to_json(wb.Sheets[s],{defval:"",raw:false})));let imported=0,skipped=0;const forced=$("importAction").value;const preview=[];for(const row of rows){const x=normalizeRow(row);x.date=excelDate(x.date);const action=forced==="AUTO"?x.action:forced;if(!x.name||!x.executionQty||!["BUY","SELL"].includes(action)){skipped++;continue}if(action==="BUY"){const brokerage=0;await dbAdd("buys",{...x,yearId:currentYearId,action:"BUY",brokerage,totalCost:x.executionQty*x.orderPrice+brokerage});preview.push(`BUY: ${x.name} | Qty ${x.executionQty} | Price ${x.orderPrice}`);imported++}else{const d=await dataSet();const h=holdingsFrom(d.buys,d.sells)[keyOf(x)];if(!h||x.executionQty>h.qty){preview.push(`SKIPPED SELL: ${x.name} | insufficient holding`);skipped++;continue}const brokerage=0,costBasis=x.executionQty*h.avg,netAmount=x.executionQty*x.orderPrice-brokerage;await dbAdd("sells",{...x,yearId:currentYearId,action:"SELL",holdingKey:h.key,brokerage,costBasis,netAmount,profit:netAmount-costBasis});preview.push(`SELL: ${x.name} | Qty ${x.executionQty} | Price ${x.orderPrice}`);imported++}}$("importMessage").textContent=`Import completed: ${imported} records imported, ${skipped} records skipped.`;$("importPreview").innerHTML=preview.slice(0,100).map(x=>`<tr><td>${escapeHtml(x)}</td></tr>`).join("");refresh()}catch(err){console.error(err);alert("Import failed: "+err.message)}};
